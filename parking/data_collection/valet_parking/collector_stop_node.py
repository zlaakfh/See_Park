#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# valet_parking/collector_stop_node.py
...
class CollectorStopNode(Node):
    """
    STOP 포함 수집 전용:
    - record_control=True이면 cmd가 0,0이든 아니든 저장
    - 4카메라 동기(sync_slop)만 맞으면 프레임 저장 + actions.csv 기록
    """

    def __init__(self):
        super().__init__('collector_stop_node')
        ...
        # EP_END 때 비어있으면 삭제할지 (원하면 True로)
        self.delete_empty_episode = bool(self.declare_parameter('delete_empty_episode', False).value)
        ...
        self.get_logger().info(f'[COLLECT_STOP] ready root={self.root} cams={self.cams}')
    ...
    def _open_episode(self, run_name: str, episode_name: str):
        ...
        self.get_logger().info(f'[COLLECT_STOP] EP_START dir={self.ep_dir}')
    ...
    def _close_episode(self):
        ...
        # 비어있으면 삭제 옵션
        if self.delete_empty_episode and self.frame_idx == 0 and self.ep_dir is not None:
            try:
                shutil.rmtree(self.ep_dir)
                self.get_logger().warn('[COLLECT_STOP] empty episode deleted')
            except Exception as e:
                self.get_logger().warn(f'[COLLECT_STOP] delete failed: {e}')
        ...
        self.get_logger().info('[COLLECT_STOP] EP_END closed')

    def _try_flush_frame(self):
        # ✅ active + record_armed만 체크 (moving 체크 없음!)
        if not self.active:
            return
    ...
    def on_event(self, msg: String):
        try:
            ...
            self.get_logger().warn(f'[COLLECT_STOP] bad /valet/event: {msg.data}')
            return
        ...
        if typ == 'EP_START':
            ...
            if not run_name or not episode_name:
                self.get_logger().warn('[COLLECT_STOP] EP_START missing run/episode')
                return
            if self.active:
                self._close_episode()
